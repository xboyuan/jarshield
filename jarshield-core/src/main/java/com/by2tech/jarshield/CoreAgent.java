...
public class CoreAgent {
    ...
    public static void premain(String args, Instrumentation inst) {
        ...
        if (StrUtils.isEmpty(pwd)) {
            ...
            if (console != null) {
                Log.debug("控制台输入");
                pwd = console.readPassword(ConsoleStyle.passwordPrompt());
            }
        }
        ...
        //还是没有获取密码，退出
        if (StrUtils.isEmpty(pwd)) {
            ConsoleStyle.printStartupFailed(
                    "Could not get the password.",
                    "Use -pwd, -pwdname ENV, jarShield.txt, or build with password=# for no-pwd mode.");
            System.exit(0);
        }
        ...
        if (passHash != null) {
            ...
            if (!StrUtils.equal(p1, p2)) {
                ConsoleStyle.printStartupFailed(
                        "Invalid password.",
                        "Check -javaagent:app.jar=-pwd *** or env JARSHIELD_PASSWORD.");
                System.exit(0);
            }
        }

        ConsoleStyle.printPasswordVerified();
        ConsoleStyle.printAgentFooter();
    ...
    public static char[] readPasswordFromFile(CmdLineOption options) {
        ...
        String configName = jarName.substring(0, jarName.length() - 3) + "jarShield.txt";
        File config = new File(path, configName);
        if (!config.exists()) {
            config = new File(path, "jarShield.txt");
        }
