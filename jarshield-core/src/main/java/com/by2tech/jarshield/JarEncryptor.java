...
import com.by2tech.jarshield.util.*;
...
public class JarEncryptor {
    ...
    static {
        ...
        aopMap.put("spring.code", "char[] c=${passchar};"
                + "is=com.by2tech.jarshield.JarDecryptor.getInstance().decryptConfigFile(this.path,is,c);");
        ...
        aopMap.put("jfinal.code", "char[] c=${passchar};inputStream=com.by2tech.jarshield.JarDecryptor.getInstance().decryptConfigFile(fileName,inputStream,c);");
...
//        });
        ...
        //[6.1]写入 Spring Boot 启动 banner（标明 JarShield 保护）
        ShieldBanner.installBanner(this.targetClassesDir);
    ...
    /**
     * 向jar文件中添加jarShield的代码
     */
    public void addjarShieldAgent() {
        ...
        thisJarPaths.forEach(thisJar -> {
            ...
            if ("jar".endsWith(this.jarOrWar) && thisJar.endsWith(".jar")) {
                List<String> includeFiles = Arrays.asList(Const.SHIELD_AGENT_FILES);
                ...
                File targetjarShieldJar = new File(this.targetLibDir, thisJarFile.getName());
                byte[] bytes = IoUtils.readFileToByte(thisJarFile);
                IoUtils.writeFile(targetjarShieldJar, bytes);
            }
            //本项目开发环境中未打包
                ...
                files.forEach(file -> {
                    ...
                    if (file.isDirectory()) {
                        targetFile.mkdirs();
                    } else if (StrUtils.containsArray(file.getAbsolutePath(), Const.SHIELD_AGENT_FILES)) {
